...
//            task.resume()
    ...
    class Cake {
        ...
        func fetchHighFilterCake(_ page: Int, completion: @escaping (Result<[MainCakeRequest]>) -> Void) {
            guard let url = URL(string: APIClient.shared.baseURL + "cakes/cake-high-filter/\(page)") else { return }
        ...
        func fetchLowFilterCake(_ page: Int, completion: @escaping (Result<[MainCakeRequest]>) -> Void) {
            guard let url = URL(string: APIClient.shared.baseURL + "cakes/cake-low-filter/\(page)") else { return }
...
//            }
    ...
    class AI {
        ...
        func imageTransFetch(_ image: Data, completion: @escaping(Result<Data>) -> Void) {
            ...
            guard let url = URL(string: "http://192.168.51.36:80/predict") else { return }
            var request = URLRequest(url: url)
            request.httpMethod = "POST"
